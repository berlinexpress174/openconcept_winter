...
from openconcept.utilities.VTOLPowerAndThrust import PowerAndThrustCal, PowerAndThrustCruiseMultiRotor
from examples.aircraft_data.Bell222B import data as acdata
from openconcept.analysis.performance.mission_profiles_eVTOL import BasicSimpleVTOLMultirotorMissionMomentumTakeoffAndCruiseOnly
...
class eVTOLModel(om.Group):
    """
    A custom model specific to an electrified helicopter
    ...
    def setup(self):
        ...
        # The maximum prop rpm is subjected to V_tip (m/s) < Mach = 0.8 at Normal Temperature and Pressure (NTP) of air
        ...
        propulsion_promotes_outputs = ['thrust','throttle']
        ...
        if flight_phase in ['takeoff','climb', 'hover', 'descent', 'landing']:
            ...
            self.add_subsystem('propmodel',PowerAndThrustCruiseMultiRotor(num_nodes=nn),
                                promotes_inputs=['*'],
                                #promotes_outputs=['*'])
...
class eVTOL_test01AnalysisGroup(om.Group):
    ...
    def setup(self):
        ...
        dv_comp.add_output_from_dict('ac|propulsion|propeller|num_blades') 
        dv_comp.add_output_from_dict('ac|propulsion|propeller|blade_chord') 
        ...
        analysis = self.add_subsystem('analysis', BasicSimpleVTOLMultirotorMissionMomentumTakeoffAndCruiseOnly(num_nodes=nn, aircraft_model=eVTOLModel),
...
def set_values(prob, num_nodes, num_rotors, spec_energy, payload, cruise_speed, design_range):
    ...
    prob.set_val('takeoff.duration', 60, units='s')
    ...
    prob.set_val('cruise.aircraftAOA', np.ones((num_nodes,)) * 4* (np.pi/180), units='rad')
    ...
    prob.set_val('landing.duration', 60, units='s')
...
def show_outputs(prob):
    ...
    vars_list = ['ac|weights|MTOW', 'takeoff.OEW','ac|payload',
                 ...
                 'ac|propulsion|motor|rating','ac|propulsion|propeller|diameter',# 'takeoff.maxproprpm',
    ...
    nice_print_names = ['MTOW', 'Takeoff OEW', 'Payload',
                        ...
                        'Final altitude', 'Final range','Battery weight','MTOW margin', 'Motor rating','Rotor Disk diameter',#'Maximum RPM',
    ...
    if plots:
        ...
        plot_trajectory(prob, x_var, x_unit, y_vars, y_units, phases,
                        ...
                        plot_title='eVTOL flight history - takeoff')
        
        # cruise only
        x_var = 'mission_time'
        x_unit = 'min'
        y_vars = ['fltcond|h', 'throttle', 'fltcond|vs', 'propmodel.batt1.SOC', 'thrust', 'range']
        y_units = ['ft', None, 'ft/min', None, 'N', 'mi']
        x_label = 'Time (min)'
        y_labels = ['Altitude (ft) (integrated)', 'Throttle (state)', 'Vertical speed (m/s) (input)', 'Battery SOC (integrated)', 'Thrust (N)', 'Range (mi)']
        phases = ['cruise']
        plot_trajectory(prob, x_var, x_unit, y_vars, y_units, phases,
                        x_label=x_label, y_labels=y_labels, marker= '-',
                        plot_title='eVTOL flight history - cruise')
...
if __name__ == "__main__":
    ...
    if run_type == 'example':
        ...
        payload = 960
        cruise_speed = 150
        run_eVTOL_type1_analysis(plots=True)
                    ...
                    for this_spec_energy in specific_energies:
                        try: 
                            ...
                            if run_type == 'optimization':
                                ...
                                # under 8 passangers
                                prob.model.add_design_var('ac|weights|MTOW', lower=100, upper = 8250,ref0 = 100, ref = 8250, units='lb')
                                prob.model.add_design_var('ac|geom|vstab|S_ref',lower=1.9, upper = 2.5, ref0 = 1.9, ref = 2.6,units='m**2')
                                prob.model.add_design_var('ac|geom|fuselage|length',lower=6, upper = 20, ref0 = 11.14, ref = 20 ,units='m')
                                #prob.model.add_design_var('ac|geom|fuselage|width',lower=1.5, upper = 20, ref0 = 3.12, ref = 20 ,units='m')
                                #prob.model.add_design_var('ac|geom|fuselage|height',lower=1.4, upper = 20, ref0 = 3.43, ref = 20 ,units='m')
                                prob.model.add_design_var('ac|propulsion|propeller|diameter',lower=42, upper = 50, ref0 = 42, ref = 50, units='ft')
                                prob.model.add_design_var('ac|propulsion|propeller|proprpm',lower=100, upper = 4000, ref0 = 100, ref = 4000,units='rpm')
                                prob.model.add_design_var('ac|weights|W_battery',lower = 10, upper = 500, ref0 = 10, ref = 500, units = 'lb')
                                prob.model.add_design_var('ac|propulsion|motor|rating',lower = 10, upper = 2500, ref0 = 25, ref = 2000, units='hp')
                                ...
                                prob.model.add_constraint('cruise.throttle', lower = 0.0, upper= 1.0, ref0 = 0, ref = 1)
                                ...
                                prob.model.add_design_var('ac|geom|vstab|S_ref',lower=9.6, ref0 = 9.6, ref = 15,units='ft**2')
                                prob.model.add_design_var('ac|geom|fuselage|length',lower=37.40, ref0 = 37.40, ref = 45 ,units='ft')
                                prob.model.add_design_var('ac|propulsion|propeller|diameter',lower= 42, ref0 = 42, ref = 50, units='ft')
                                prob.model.add_design_var('ac|propulsion|propeller|proprpm',lower= 348, ref0 = 800, ref = 2000,units='rpm')
                            ...
                            """
                            # SLSQP
                            #outfile_to_save = 'case_'+'Heli_'+'SLSQP_'+'range'+str(design_range)+'_'+'battary'+str(spec_energy)+'_'+'payload'+str(payload)+'_'+'cruise'+str(cruise_speed)+'_'+'Rotor'+str(num_rotors)+'.out'
                            ...
                            """
                            if write_logs:
                                filename_to_save = 'case_'+'Bell222B_'+'SNOPT_'+'range'+str(design_range)+'_'+'battary'+str(spec_energy)+'_'+'payload'+str(payload)+'_'+'cruise'+str(cruise_speed)+'_'+'Rotor'+str(num_rotors)+'.sql'
                                filename_failed_to_save = 'case_'+'Bell222B_'+'SNOPT_'+'range'+str(design_range)+'_'+'battary'+str(spec_energy)+'_'+'payload'+str(payload)+'_'+'cruise'+str(cruise_speed)+'_'+'Rotor'+str(num_rotors)+'_failed.sql'
